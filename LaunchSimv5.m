...
OB.launchE = 3;  %launch energy in joules
OB.pos0 = [BD.earthLPos-.5*OB.dim , OB.dim]; %m
...
GUI.field.ang.ValueChangedFcn = @updateCursor;
    ...
    function makeLander(x0, y0, vx0,vy0,t0)
        ...
        function tableTrajectory(x0, y0, vx0, vy0, t0)
            ...
            %for seperate event cases and can backtack its calculation to a
            %point that misses the fact that the object has exited the
            %funnel system
            R = rTitan(U(:,4),U(:,5),BD);
            rcheck = R < .45;
            index = find(rcheck,1);
        ...
        function funnelTrajectory(x0, y0, vx0, vy0, t0)
            ...
            function dudt = funnelOde(t,u, vars)
                ...
                ddr = 1/m*(-N*sinphi+fric(1))+r*dth^2;
...
%                 ddth = ddth + aFric(1);               
        ...
        function plotTrajectory()
            ...
            R = rTitan (X,Y,BD);
            offset = .002; %lift the thicker line partially off surace for visibility
    ...
    for k = 1:maxIndex
        ...
        if(oLaunch )
            if(k<oT(end))
                ...
                orbgfx.Position(1:2)=oPos(k,:)-.5*OB.dim;
            end            
        end
        if(k<T(end))
    ...
    function launchOrbiter(btn, event)
        ...
        %find when x-value in the orb path is above the trigger val
        lindex = find(Orb.path(:,4)>GUI.field.td.Value ,1);
        if(~isempty(lindex));
        ...
        function orbitalLaunch(data)
            [vx, vy] = pol2cart(GUI.field.lang.Value*pi/180, GUI.field.lvel.Value);
...
lbltd = uilabel(GUI.grid, 'Text', 'Trigger Distance');
...
GUI.labels = [lblxpos lblypos lblvel lblang lbltd lbllvel lbllang lblfric lblorbvel];
...
fields = [GUI.field.xpos  GUI.field.ypos GUI.field.vel GUI.field.ang...
          GUI.field.td GUI.field.lvel GUI.field.lang GUI.field.fric GUI.field.orbvel];   
